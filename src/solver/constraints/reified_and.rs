...
/// Enforces `B_out <==> (B_in_1 AND B_in_2 AND ...)`
#[derive(Debug, Clone)]
pub struct ReifiedAndConstraint<S: DomainSemantics + std::fmt::Debug>
where
...
impl<S: DomainSemantics + std::fmt::Debug> ReifiedAndConstraint<S>
where
    S::Value: From<StandardValue> + PartialEq,
{
...
impl<S: DomainSemantics + std::fmt::Debug> Constraint<S> for ReifiedAndConstraint<S>
where
    S::Value: From<StandardValue> + PartialEq,
{
    ...
    fn descriptor(&self) -> ConstraintDescriptor {
        ...
        ConstraintDescriptor {
            name: "ReifiedAndConstraint".to_string(),
    ...
    fn revise(
        ...
        let mut any_input_is_false = false;
        let mut all_inputs_are_true = true;
        for b_in_var in &self.b_in {
            let b_in_domain = solution.domains.get(b_in_var).unwrap();
            if b_in_domain.is_singleton() {
                if b_in_domain.get_singleton_value().unwrap() == false_val {
                    any_input_is_false = true;
        ...
        if any_input_is_false {
            // If any input is false, output must be false
...
mod tests {
    ...
    fn revise_propagates_from_input_to_output_false() {
        ...
        let b_in1 = 1;
        let b_in2 = 2;
        let constraint = ReifiedAndConstraint::<TestSemantics>::new(b_out, vec![b_in1, b_in2]);

        let domains = im::hashmap! {
            b_out => bool_domain(),
            b_in1 => domain_from_slice(&[false]),
            b_in2 => domain_from_slice(&[true]),
        };
    ...
    #[test]
    fn revise_propagates_from_output_to_inputs_true() {
        let b_out = 0;
        let b_in1 = 1;
        let b_in2 = 2;
        let constraint = ReifiedAndConstraint::<TestSemantics>::new(b_out, vec![b_in1, b_in2]);
    ...
    fn revise_does_nothing_if_insufficient_info() {
        ...
        // Case 1: Output is false, inputs are ambiguous
        ...
        // Case 2: Output is ambiguous, one input is true, one is ambiguous
